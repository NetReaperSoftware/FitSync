...
export default function WorkoutTrackerScreen(): React.JSX.Element {
  ...
  // Fetch exercises, routines, and folders from database, and restore active workout
  useEffect(() => {
    fetchExercises();
    fetchFolders();
    fetchRoutines();
  ...
  const saveWorkoutToDatabase = async (workout: WorkoutSession) => {
    try {
      const user = await supabase.auth.getUser();
      if (!user.data.user) {
        ...
        for (const set of completedSets) {
          const { error: setError } = await supabase
            ...
            .insert({
              ...
              created_at: set.completedAt?.toISOString() || new Date().toISOString()
            });
  ...
  const finishWorkout = async () => {
    if (!currentWorkoutId || !workoutStartTime) return;
    ...
    try {
      ...
      console.log('Workout completed and saved:', currentWorkoutId);
  ...
  const addExerciseToWorkout = async (exercise: DatabaseExercise) => {
  ...
  const toggleActiveWorkoutSetCompletion = useCallback((exerciseId: string, setId: string) => {
    ...
    // Debounced save to storage for set completion
    if (currentWorkoutId && workoutStartTime) {
      const workout: WorkoutSession = {
        id: currentWorkoutId,
        startTime: workoutStartTime,
